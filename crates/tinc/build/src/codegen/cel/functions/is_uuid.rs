...
pub(crate) struct IsUuid;

// this.isUuid(arg) -> arg in this
    ...
    fn compile(&self, ctx: CompilerCtx) -> Result<CompiledExpr, CompileError> {
        ...
        match this {
            CompiledExpr::Constant(ConstantCompiledExpr { value }) => {
                Ok(CompiledExpr::constant(CelValue::cel_is_uuid(value)?))
            }
...
mod tests {
    ...
    use crate::codegen::cel::functions::{Function, IsUuid};
    use crate::codegen::cel::types::CelType;
    ...
    fn test_is_uuid_syntax() {
        ...
        insta::assert_debug_snapshot!(IsUuid.compile(CompilerCtx::new(compiler.child(), Some(CompiledExpr::constant(CelValue::String("xdd".into()))), &[])), @r"
        Ok(
            Constant(
        ...
        insta::assert_debug_snapshot!(IsUuid.compile(CompilerCtx::new(compiler.child(), Some(CompiledExpr::constant(CelValue::String("e470fd2a-f176-40cd-9ea1-9858deade3e1".into()))), &[])), @r"
        Ok(
            Constant(
                ConstantCompiledExpr {
                    value: Bool(
        ...
        insta::assert_debug_snapshot!(IsUuid.compile(CompilerCtx::new(compiler.child(), Some(CompiledExpr::constant(CelValue::List(Default::default()))), &[
    ...
    fn test_is_uuid_runtime() {
        ...
        insta::assert_snapshot!(postcompile::compile_str!(
            ...
            quote::quote! {
                ...
                #[test]
                fn test_is_hostname() {
                    assert_eq!(is_uuid("e470fd2a-f176-40cd-9ea1-9858deade3e1").unwrap(), true);
                    assert_eq!(is_uuid("not-a-uri").unwrap(), false);
                }
            },
        ));
    }
